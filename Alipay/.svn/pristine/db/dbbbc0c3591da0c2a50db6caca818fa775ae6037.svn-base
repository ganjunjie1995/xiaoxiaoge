...
<body>
	<div>
		<span style="padding-left: 440px">我的金豆:0.00元</span>
	</div>
	...
	<div style="width: 30%;
      ...
      ">
		...
		<form action="/gb/gold.do" method="get">
		...
		<p>  <%--<c:if test="${page.currPage <= 1 }"></c:if>--%>
			<a href="${pageContext.request.contextPath}/gb/gold.do" >首页</a>


			<!-- 上页按钮，跳转到上一页 -->
			<c:if test="${pagination.pageNo  <= 1 }">
				<a href="${pageContext.request.contextPath}/gb/gold.do" >上页</a>
			</c:if>

			<c:if test="${pagination.pageNo > 1 }">
				<a href="${pageContext.request.contextPath}/gb/gold.do?pageNo=${pagination.pageNo-1 }" >上页</a>
			</c:if>

			<!-- 下页按钮，跳转到下一页 -->
			<c:if test="${pagination.pageNo >= pagination.totalPage }">
				<a href="${pageContext.request.contextPath}/gb/gold.do" >下页</a>
			</c:if>
			<c:if test="${pagination.pageNo < pagination.totalPage }">
				<a href="${pageContext.request.contextPath}/gb/gold.do?pageNo=${pagination.pageNo+1 }" >下页</a>
			</c:if>

			<!-- 末页按钮，跳转到最后一页 -->
			<a href="${pageContext.request.contextPath}/gb/gold.do?pageNo=${pagination.totalPage }" >末页</a>
		</p>
	</div>
</body>
</html>