...
<body>
	<div>
		<span style="padding-left: 440px">可用余额:0.00元</span><span style="padding: 35px">冻结余额:0.00元</span>
	</div>
	
	<div style="padding-left: 440px;padding-top: 10px;padding-bottom: 20px">最近交易记录</div>
		...
		<table border="0">
			...
			<c:if test="${user.status== '0' }">
				...
				<td style="padding-left: 70px">
					...
					<a href="${pageContext.request.contextPath}/pureController/deleteSelection.do?code=${user.code}" style="text-decoration: none">删除</a>
				</td>
	...
	<div style="width: 30%;
      ...
      ">
		...
		<form action="/pureController/purse.do" method="get">
		...
		<p>  <%--<c:if test="${page.currPage <= 1 }"></c:if>--%>
			<a href="${pageContext.request.contextPath}/pureController/purse.do" >首页</a>


			<!-- 上页按钮，跳转到上一页 -->
			<c:if test="${pagination.pageNo  <= 1 }">
			<a href="${pageContext.request.contextPath}/pureController/purse.do" >上页</a>
			</c:if>

			<c:if test="${pagination.pageNo > 1 }">
				<a href="${pageContext.request.contextPath}/pureController/purse.do?pageNo=${pagination.pageNo-1 }" >上页</a>
			</c:if>

			<!-- 下页按钮，跳转到下一页 -->
			<c:if test="${pagination.pageNo >= pagination.totalPage }">
				<a href="${pageContext.request.contextPath}/pureController/purse.do?" >下页</a>
			</c:if>
			<c:if test="${pagination.pageNo < pagination.totalPage }">
				<a href="${pageContext.request.contextPath}/pureController/purse.do?pageNo=${pagination.pageNo+1 }" >下页</a>
			</c:if>

			<!-- 末页按钮，跳转到最后一页 -->
			<a href="${pageContext.request.contextPath}/pureController/purse.do?pageNo=${pagination.totalPage }" >末页</a>
		</p>
	</div>
</body>
</html>